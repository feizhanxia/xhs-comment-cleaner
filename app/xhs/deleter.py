...
class CommentDeleter:
    ...
    def delete_comment(self, comment: Comment) -> None:
        ...
        self.page.goto(comment.note_url, wait_until="domcontentloaded", timeout=30_000)
        detect_risk_control(self.page)
        target = comment_locator(self.page, comment.comment_id)
        if target is None:
            raise CommentNotFound("无法按评论 ID 定位目标评论")

        author = target.locator(selectors.COMMENT_AUTHOR_LINK).first
        href = author.get_attribute("href", timeout=3_000)
        ...
        # The destructive item must be exposed as a menu item opened from this
        # exact comment. A global text match could select an unrelated delete UI.
        delete = self.page.get_by_role("menuitem", name=selectors.DELETE_TEXT, exact=True).last
        if not delete.is_visible(timeout=3_000):
            raise UnsupportedPageState("找不到删除按钮")
        delete.click(timeout=5_000)

        for label in selectors.CONFIRM_DELETE_TEXT:
            confirm = self.page.get_by_role("button", name=label, exact=True).last
            try:
                if confirm.is_visible(timeout=700):
        ...
        detect_risk_control(self.page)
        if not verify_deleted(self.page, comment):
